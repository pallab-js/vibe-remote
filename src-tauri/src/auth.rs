...
impl PublicIdentity {
    ...
    pub fn verifying_key_bytes(&self) -> Vec<u8> {
        base64::Engine::decode(&base64::engine::general_purpose::STANDARD, &self.verifying_key_b64)
            .unwrap_or_default()
    }
}
...
#[allow(dead_code)] // Reserved for future application-layer authentication
impl NoiseHandshake {
    ...
    pub fn new_initiator(identity: PeerIdentity, remote_public_key_b64: &str) -> VibeResult<Self> {
        ...
        let remote_key_bytes = BASE64.decode(remote_public_key_b64)
    ...
    pub fn write_message(&mut self, payload: &[u8], out: &mut Vec<u8>) -> VibeResult<bool> {
        ...
        let written = self.state.write_message(&payload, out)
    ...
    pub fn read_message(&mut self, message: &[u8], out: &mut Vec<u8>) -> VibeResult<bool> {
        ...
        let written = self.state.read_message(message, out)
