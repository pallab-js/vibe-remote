...
use vibe_remote_lib::{
    capture::{CaptureStream, CaptureConfig},
    encoder::FrameEncoder,
    input::{InputHandler, MouseEvent, KeyboardEvent, VirtualKey},
    transport::{QuicTransport, QuicConfig},
...
async fn test_frame_encoding_pipeline() {
    ...
    let encoded = encoder.encode_frame(&frame_data, width, height, 0)
        .expect("Frame encoding failed");
    
    let encode_duration = start.elapsed();
    
    // Verify encoding reduced size
    assert!(encoded.data.len() < frame_data.len(), 
        "Encoded frame should be smaller than raw");
    
    // Log compression ratio
    let ratio = frame_data.len() as f64 / encoded.data.len() as f64;
    println!("Compression ratio: {:.1}:1 ({:.1}ms)", 
             ratio, encode_duration.as_millis());
...
async fn test_input_handler() {
    let handler = InputHandler::new()
        .expect("Failed to create input handler");
    
    // Test mouse move
    handler.handle_mouse_event(MouseEvent::Move { x: 100, y: 200 })
        .expect("Mouse move failed");
    
    // Test keyboard input
    handler.handle_keyboard_event(KeyboardEvent::Text { 
        text: "test".to_string() 
    }).expect("Keyboard input failed");
    
    // Test special keys
    handler.handle_keyboard_event(KeyboardEvent::KeyDown { 
...
async fn test_peer_identity() {
    ...
    // Generate identity
    let public = PeerIdentity::load_or_generate(&key_path, "TestUser".to_string())
    ...
    let public2 = PeerIdentity::load(&key_path)
        .expect("Failed to load identity");
    
    assert_eq!(public.verifying_key_b64, public2.public_identity().verifying_key_b64);
    
    // Test signature verification
    let message = b"test message";
    let identity = PeerIdentity::load(&key_path).unwrap();
    ...
    let result = PeerIdentity::verify_signature(
        &public.verifying_key_b64,
        message,
        &signature_b64,
    );
...
async fn test_nat_discovery() {
    use vibe_remote_lib::nat_traversal::default_nat_traversal;
    
    let mut nat = default_nat_traversal();
    
    // This test may fail in CI environments without network access
    match nat.discover_public_address() {
        Ok(addr) => {
            println!("Public address discovered: {}", addr);
            assert!(addr.port() > 0);
        }
        Err(e) => {
            println!("STUN discovery skipped (network unavailable): {}", e);
        }
    }
}
...
async fn benchmark_frame_encoding() {
    ...
    for _ in 0..iterations {
        encoder.encode_frame(&frame_data, width, height, 0)
    ...
    println!("Encoding performance: {:.1} FPS ({}x{})", 
             fps, width, height);
    
    // Should achieve at least 15 FPS on modern hardware
    assert!(fps > 15.0, "Encoding too slow: {:.1} FPS", fps);
}
...
async fn benchmark_quic_throughput() {
    ...
    let (server, mut client) = create_local_tunnel()
        .await
    ...
    for i in 0..iterations {
        ...
        // In production, would send via QUIC
        let _bytes = data.len();
    }
...
async fn test_end_to_end_capture() {
    ...
    let mut receiver = stream.get_primary_stream()
        .await
    ...
    loop {
        ...
        if let Some(frame) = receiver.recv().await {
            ...
            println!("Encoded: {} bytes (ratio: {:.1}:1)", 
                     encoded.data.len(),
